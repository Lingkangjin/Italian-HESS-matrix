...
class HESS():
    ...
    def load(self,):
        file_path=os.path.join(os.path.dirname(os.getcwd()), 'Input_data', 'reg_2022_cons.csv')
        ...
        residenza = "Tutti"
        de = d[cond_residenza & cond_mercato & (
            d["Anno Mese"] == mese) & (d["Working Day"] == "DOM")]

        # drange[0].weekday()

        de['Prelive medio Orario Regionale (kWh)'] = de['Prelive medio Orario Regionale (kWh)'].str.replace(
        ...
        for i in days:
            if i.date() in holiday_dates:  # check holidays  and substitute as sunday
                Power.extend(d[cond_residenza & cond_mercato & (d["Anno Mese"] == mesi[i.month-1]) & (d["Working Day"]
                             == "DOM")]["Prelive medio Orario Regionale (kWh)"].str.replace(',', '.').astype(float))
                ...
                Power.extend(d[cond_residenza & cond_mercato & (d["Anno Mese"] == mesi[i.month-1]) & (
                    d["Working Day"] == "SAB")]["Prelive medio Orario Regionale (kWh)"].str.replace(',', '.').astype(float))
                giorni.extend(["Saturday"]*24)
            else:
                Power.extend(d[cond_residenza & cond_mercato & (d["Anno Mese"] == mesi[i.month-1]) & (d["Working Day"]
                             == "Giorno_feriale")]["Prelive medio Orario Regionale (kWh)"].str.replace(',', '.').astype(float))
