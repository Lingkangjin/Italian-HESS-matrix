...
class HESS():
    ...
    def load(self,):
        ...
        de = d[cond_residenza & cond_mercato & (
            d["Anno Mese"] == mese) & (d["Working Day"] == "Domenica")]
        ...
        for i in days:
            if i.date() in holiday_dates:  # check holidays  and substitute as sunday
                Power.extend(d[cond_residenza & cond_mercato & (d["Anno Mese"] == mesi[i.month-1]) & (d["Working Day"]
                             == "Domenica")]["Prelive medio Orario Regionale (kWh)"].str.replace(',', '.').astype(float))
                ...
                Power.extend(d[cond_residenza & cond_mercato & (d["Anno Mese"] == mesi[i.month-1]) & (
                    d["Working Day"] == "Sabato")]["Prelive medio Orario Regionale (kWh)"].str.replace(',', '.').astype(float))
                giorni.extend(["Saturday"]*24)
            else:
                Power.extend(d[cond_residenza & cond_mercato & (d["Anno Mese"] == mesi[i.month-1]) & (d["Working Day"]
                             == "Giorno feriale")]["Prelive medio Orario Regionale (kWh)"].str.replace(',', '.').astype(float))
