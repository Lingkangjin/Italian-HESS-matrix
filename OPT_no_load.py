...
def pyomo_model(lf):
    ...
    for h in hlist:
        ...
        model.storage_const.add(SOC_min* model.BESS_cap<=model.BESS_st[h] ) #Min SOC
...
bars = ax.bar(df_plot["lf EZ"].index, df_plot["lf EZ"],edgecolor="k",label="Load factor",alpha=0.9)
for bar in bars:
